      ...
      <div className="max-w-7xl mx-auto">
        ...
        <motion.div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8" variants={containerVariants} initial="hidden" animate={inView ? 'visible' : 'hidden'}>
          {dict.items.map((feature: any, index: number) => (
            <motion.div key={index} className="card group" variants={itemVariants} whileHover={{ y: -10, boxShadow: '0 20px 40px rgba(139, 92, 246, 0.2)' }}>
              ...
              <motion.div className="absolute inset-0 rounded-xl border border-purple-500/0 group-hover:border-purple-500/50" initial={{ opacity: 0 }} whileHover={{ opacity: 1 }} />
            </motion.div>
          ))}
        </motion.div>
      </div>
